  ...
  (config: InternalRequestConfig) => {
    ...
    if (!config.ignoreDuplicates) {
      config.cancelToken = new axios.CancelToken((cancelFn) => {
        ...
        // 如果已请求，则取消重复请求
        // TODO: 临时解决文件查询请求重复问题
        if (pendingPool.has(config.url) && !config.url.startsWith('/coderepo/web/v1/file/') && pendingPool.get(config.url)?.method === config.method) {
...
const responseInterceptorId = request.interceptors.response.use(
  ...
  (err: AxiosError) => {
    ...
    if (
      (err.config?.method === 'post' && err.config.url?.endsWith('/config/subs') && (err.response?.data as any)?.exist) ||
